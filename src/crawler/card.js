...
const bindCard = ($, cardId, single, containerId, foreign) => {
    ...
    let set = {};
    if (single || (!single &&  !foreign)) {
        set.key = $($($(config.cardContainer(cardProperty.SET, single, containerId)).eq(0)).find('img')).attr('src').split('set=')[1].split('&')[0];
        set.name = $($(config.cardContainer(cardProperty.SET, single, containerId)).eq(1)).text().trim();
    } else {
        set.name = $($(config.cardContainer(cardProperty.SET, single, containerId)).eq(0)).text().trim();
    }
...
const get = (cardId, { multiverseId, language }) => {
    return new Promise((resolve, reject) => {
        c.queue([{
            uri: config.cardDetailsUrl(cardId, !!language),
                    ...
                    try {
                        ...
                        if($(`${config.cardComponentContainer} .cardDetails`).length == 1) {
                            ...
                            $(config.cardComponentContainer).toArray().forEach((e) => {
                                ...
                                const face = bindCard($, cardId, false, id.substring(id.indexOf('SubContent_ctl')+14, (id.length - 17)), !!language);
